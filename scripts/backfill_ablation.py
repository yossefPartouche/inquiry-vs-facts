...
def main():
    headline_rows = [json.loads(l) for l in open("results/gen_number_theory_headline.jsonl")]
    ablation_rows = [json.loads(l) for l in open(OUTPUT_PATH)]

    pool = [r for r in headline_rows if r["condition"] == "A" and r["model"] == MODEL_KEY and r.get("correct")]
    already_done = {r["problem_id"] for r in ablation_rows}
    missing = [r for r in pool if r["problem_id"] not in already_done]

    print(f"Backfilling {len(missing)} problems for ablation")
    ...
    for r in missing:
        ...
        if doctored_prompt is None:
            skipped += 1
            print(f"  skip {r['problem_id']}: could not corrupt")
            continue
        ...
        written += 1
        print(f"  done {r['problem_id']} ({written}/{len(missing)})")
